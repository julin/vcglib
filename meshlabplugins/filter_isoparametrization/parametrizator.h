...
//		MyTriEdgeFlip::EType()=EType;
    pecp.EType()=_EType;
        ...
        typename MeshType::CoordType transl=-mesh->bbox.Center();
     ...
     FinalOptimization(&pecp);
        ...
        CoordType ntransl = CoordType::Construct(-transl);

        ScaleMesh(final_mesh,1.0/factor);
        TranslateMesh(final_mesh,ntransl);
        ScaleMesh(base_mesh,1.0/factor);
        TranslateMesh(base_mesh,ntransl);
        return Done;
    }
